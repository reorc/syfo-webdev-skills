...
if (!existsSync(manifestPath)) {
  ...
  const template = section(manifest, "template");
  ...
  const checks = [
    ...
    ["manifest-unified-template", /^\s*id:\s*web-unified\s*$/m, "Unified Apps must declare template.id: web-unified."],
  ...
  if (scalar(template, "id") !== "web-unified") add("error", "manifest-template-contract", "syfo.yaml", "template.id must be web-unified.");
  const databaseRequired = scalar(database, "required");
  if (!["true", "false"].includes(databaseRequired)) add("error", "manifest-database-required", "syfo.yaml", "database.required must explicitly be true or false.");
