...
The daemon CLI currently accepts only these internal compatibility presets:

- `template=unified,preset=site`
- `template=unified,preset=app`

The daemon deterministically maps `site` to the complete Core pair `preset=site,database=none` and `app` to `preset=app,database=tidb`. These values are internal and must not become user-facing website types. The CLI must not expose `--database`. A new Syfo-hosted website defaults to `site/none` when TiDB has not been explicitly requested or approved. The words “app,” “web app,” and “Syfo Hosted App” do not imply `preset=app`. `preset=app` requires one informed explicit consent to TiDB provisioning: either an initial explicit TiDB request or a yes after the TiDB disclosure. That choice is the confirmation; never ask twice. The confirmed init command must include `--confirm-tidb`, while `preset=site` must never include that flag. Legacy template commands remain unchanged and must not accept or derive unified fields.
