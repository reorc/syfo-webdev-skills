...
The daemon deterministically maps `site` to the complete Core pair `preset=site,database=none` and `app` to `preset=app,database=tidb`. The CLI must not expose `--database`. Missing or invalid presets are errors. `preset=app` requires disclosure and confirmation before TiDB provisioning, and the confirmed init command must include `--confirm-tidb`; `preset=site` must never include that flag. Legacy template commands remain unchanged and must not accept or derive unified fields.
