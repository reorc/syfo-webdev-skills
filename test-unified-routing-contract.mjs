...
test('unified create contract accepts only frozen pairs', () => {
  ...
  assert.match(unified, /Never ask twice when the user already approved TiDB provisioning/);
  assert.match(unified, /Never pass `--confirm-tidb` for `preset=site` or without informed explicit TiDB consent/);
});
...
test('legacy aliases preserve flow and require explicit upgrade consent', () => {
  ...
  assert.match(unified, /Create and maintain websites hosted by Syfo/i);
...
test('existing unified site database enable uses one consent and preserves lifecycle boundaries', () => {
  ...
  assert.match(unified, /Modify the same original repository for TiDB usage, including `database.required: true`/);
...
test('Syfo hosting defaults to no database while persistence asks only for TiDB consent', () => {
  assert.match(unified, /When the user selects Syfo hosting without explicitly requesting or approving TiDB, default to the website-without-database path/);
  assert.match(unified, /If the requested functionality requires persistence.*one focused consent question/s);
  assert.match(unified, /Syfo will provision a TiDB cloud database for it\. Should I enable TiDB\?/);
...
test('evals distinguish explicit TiDB consent from generic Syfo hosting', async () => {
  ...
  assert.match(ambiguousCreate.expected_output, /legacy wording.*does not authorize TiDB/i);
});
