...
function parseArguments(args) {
  const options = { template: process.env.SYFO_FULLSTACK_TEMPLATE_DIR || '', reuseInstall: false };
  ...
  if (!options.template) {
    throw new Error('Pass --template <official-web-fullstack-checkout> or set SYFO_FULLSTACK_TEMPLATE_DIR.');
  }
  return options;
}
...
export async function validateFullstackTemplateContract(template) {
  ...
  if (!packageJson.dependencies?.mysql2) throw new Error('Official fullstack template must keep its TiDB-compatible driver.');
...
export async function runFullstackCanary(options) {
  const source = resolve(options.template);
  const contract = await validateFullstackTemplateContract(source);
  ...
  try {
    if (!options.reuseInstall) {
      scratch = await mkdtemp(join(tmpdir(), 'syfo-fullstack-template-canary-'));
    ...
    if (!budget.ok) {
      throw new Error(`Fullstack artifact exceeds Builder budget: ${JSON.stringify(budget.violations)}`);
    }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const result = await runFullstackCanary(parseArguments(process.argv.slice(2)));
